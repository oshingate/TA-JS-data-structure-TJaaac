...
let noOfPeoples = [];
for (let i of got.houses) {
  nameOfHou.push(i.name);
  noOfPeoples.push(i.people.length);
}
for (let i = 0; i < nameOfHou.length; i++) {
  obj[nameOfHou[i]] = noOfPeoples[i];
}
console.log(obj);
...
for (let i of got.houses) {
  for (let j of i.people) {
    allDescription.push(j.description);
  }
}
...
let bigDescription = [];
for (let i of allDescription) {
  if (i.length > 30) {
    bigDescription.push(i);
  }
}
